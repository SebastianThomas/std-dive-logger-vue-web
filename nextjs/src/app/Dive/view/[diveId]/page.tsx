...
function SummaryItem({ label, value }: { label: string; value: string }) {
    return (
        <div className="bg-white bg-opacity-90 rounded-xl shadow-md px-3 py-2 flex flex-col items-center min-w-[120px]">
...
function ViewContent({ diveId }: { diveId: number }) {
    ...
    const [myDives, setMyDives] = useState<Dive[]>([]);
    ...
    useEffect(() => {
        const fetchMyDives = async () => {
            ...
            try {
                const res = await getWithToken("/v1/dives?page=0&sortCol=NUMBER&sortDirection=ASCENDING");
    ...
    const handleLinkDive = async (buddyDiveId: number) => {
        if (!dive) return;
        try {
            await postWithToken(`/v1/dives/${diveId}/link?buddyDiveId=${buddyDiveId}`);
    ...
    /* TODO */
    const filteredMyDives = myDives.filter(d =>
    ...
    const bottomTimeAccTemp = summaries.map(s => s.bottomTime).map(b => parseISODuration(b)).reduce((acc, b) => ({
    ...
    const bottomTimeAcc = {
        ...
        minutes: Math.floor(bottomTimeAccTemp.seconds / 60) + bottomTimeAccTemp.minutes % 60,
                ...
                <div className="space-y-6 md:space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-md p-4 md:p-6 flex flex-col">
                        ...
                        <p className="text-gray-500 text-sm">
                            {dive.site.name} · {firstProfile?.start ? new Date(firstProfile.start).toLocaleString() : "No start date"}
                        </p>
                    </div>
                            ...
                            <div className="bg-white rounded-xl shadow-lg p-6 w-[90%] max-w-lg max-h-[80vh] overflow-auto">
                                ...
                                <ul className="space-y-2">
                                    {filteredMyDives.map(d => (
                                        <li key={d.id} className="flex justify-between items-center bg-gray-50 p-2 rounded hover:bg-gray-100 cursor-pointer">
                                            <div>
                                                <p className="text-sm font-medium">{d.customIdentifier}</p>
                                                <p className="text-xs text-gray-500">
                                                    #{d.number} · {d.site.name} · {firstProfile?.start ? new Date(firstProfile.start).toLocaleDateString() : "No date"}
                                                </p>
                                            </div>
                                            <button
                                    ...
                                    {filteredMyDives.length === 0 && <li className="text-sm text-gray-400">No dives found</li>}
                                </ul>
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="w-full md:w-1/5 h-[200px] rounded-lg overflow-hidden shadow-sm border">
                            <MapDisplay
                                sites={[{
                                    site: {
                                        id: dive.site.id!,
                        ...
                        <div className="w-full md:w-4/5 flex flex-col gap-6">
                            ...
                            {firstProfile && summary && (
                                <section className="flex flex-wrap justify-center gap-4 md:gap-6 mb-4">
                                    <SummaryItem label="Max Depth" value={`${summary.maxDepth} m`} />
                                    <SummaryItem label="Avg Depth" value={`${summary.averageDepth?.toFixed(2)} m`} />
                                    <SummaryItem label="Bottom Time" value={showDuration(bottomTimeAcc)} />
                                    {summary.surfaceInterval && <SummaryItem label="Surface Interval" value={summary.surfaceInterval} />}
                                    {summary.avgAscentRate && <SummaryItem label="Avg Ascent Rate" value={`${summary.avgAscentRate} m /min`} />}
                                    {summary.o2Toxicity && <SummaryItem label="Oxygen Toxicity Unit" value={`${summary.o2Toxicity}%`} />}
                                </section>
                            )}
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                                ...
                                <div className="bg-white rounded-xl shadow-md p-4">
                                    <h2 className="font-semibold mb-2 text-gray-700 text-sm">Gases</h2>
                                    <ul className="text-xs text-gray-600 space-y-1">
                                        {firstProfile?.measurements
                                            ...
                                            .map((gas, i) => (
                                                <li key={i}>
                                                    O₂ {((gas.o2 ?? 0) * 100).toFixed(2)}% ·
                                                    He {((gas.he ?? 0) * 100).toFixed(2)}% ·
                                                    N₂ {((gas.n2 ?? 0) * 100).toFixed(2)}%
                                ...
                                {/* Dive Computer */}
                                <div className="bg-white rounded-xl shadow-md p-4">
                                    <h2 className="font-semibold mb-2 text-gray-700 text-sm">Dive Computer</h2>
                                    <p className="text-xs text-gray-600">{firstProfile?.diveComputer.manufacturer.name} — {firstProfile?.diveComputer.customIdentifier}</p>
                                </div>

                                {/* Buddies */}
                                <div className="bg-white rounded-xl shadow-md p-4">
                                    <h2 className="font-semibold mb-2 text-gray-700 text-sm">Buddies</h2>
                                    {(!dive.namedBuddies.length && !dive.buddiesDives?.length) ? (
                                        ...
                                        <ul className="text-xs text-gray-600 list-disc list-inside space-y-1">
                                            ...
                                            {dive.buddiesDives?.map(({ buddy, diveId }) => (
                                                <li key={`linked-${buddy.id}`} className="flex items-center gap-1">
                    ...
                    <div className="w-full flex justify-center mt-6">
                        <div className="bg-white rounded-xl shadow-md w-full max-w-[600px] flex flex-col">
                            <div className="flex justify-between items-center p-4">
                                <h2 className="font-semibold text-gray-700 text-sm">Dive Profile</h2>
                                <button
...
// Main page component
// ------------------------
export default function ViewDivePage({ params }: { params: Promise<{ diveId: number }> }) {
