...
export default function SharePopover({
    ...
    async function fetchAllReaders() {
        try {
            ...
            const resUser = await getWithToken(`/v1/dives/${diveId}/readers`)
            setReaders(resUser.data.result)
            const resGroup = await getWithToken(`/v1/dives/${diveId}/group-readers`)
            setGroupReaders(resGroup.data)
        } catch (err: any) {
            console.error(err)
        }
        ...
        >
            <div className="flex flex-col h-full min-h-0 max-h-70 w-70">
                ...
                <div className="flex-1 overflow-y-auto px-4 py-2">
                    ...
                    {view === "add-person" && <AddPerson diveId={diveId} fetchAllReaders={fetchAllReaders} />}
                    {view === "add-group" && <AddGroup diveId={diveId} fetchAllReaders={fetchAllReaders} />}
                </div>
            </div>
        </ Popover>
    );
...
function Overview({
    ...
    async function deletePerson(id: number) {
        try {
            if (diveId == undefined) return;
            const res = await deleteWithToken((`/v1/dives/${diveId}/readers`), [id], {});
            fetchAllReaders();
        } catch (err: any) {
    ...
    async function deleteGroup(id: number | undefined) {
        try {
            if (diveId == undefined) return;
            const res = await deleteWithToken((`/v1/dives/${diveId}/group-readers?groupId=${id}`), {}, {});
            fetchAllReaders();
        } catch (err: any) {
...
// ---- Add person view ----
function AddPerson({ diveId, fetchAllReaders }: { diveId: number | undefined, fetchAllReaders: () => void }) {
    const { postWithToken } = useApi();

    async function handleSubmitAddPerson(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const res = await postWithToken((`/v1/dives/${diveId}/readers`), diveBuddies, {}, "application/json");
    ...
    return (
        <form className="flex flex-col gap-4" onSubmit={handleSubmitAddPerson}>
            <AutocompleteInput suburl="user" label="enter username" name="diveBuddy" multiple={true} />
            <input type="submit" value="Add buddy" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg cursor-pointer hover:bg-sky-600 transition-colors duration-200" />
        </form>
    );
}

// ---- Add group view  ----
function AddGroup({ diveId, fetchAllReaders }: { diveId: number | undefined, fetchAllReaders: () => void }) {
    const { postWithToken } = useApi();
    ...
    async function handleSubmitAddGroup(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const res = await postWithToken((`/v1/dives/${diveId}/group-readers`), diveGroup, {}, "application/json");
    ...
    return (
        <form className="flex flex-col gap-4" onSubmit={handleSubmitAddGroup}>
            <AutocompleteInput suburl="group" label="enter group name" name="groupName" multiple={false} />
            <input type="submit" value="Add groups" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg cursor-pointer hover:bg-sky-600 transition-colors duration-200" />
        </form>
    );
}
