    ...
    <div class="modal-card">
      ...
      <div class="modal-body">
        <p class="hint">
          ...
          PNG, JPEG, or WEBP, up to 1 MB — it's scaled to fit, so a simple square/landscape image
          works best.
        </p>

        <div class="preview-row">
          <div class="preview-box">
            <img :src="previewUrl ?? currentIconUrl ?? defaultIconUrl" alt="Icon preview" />
          </div>
          <div class="form-group">
            <label for="icon-file">Icon file</label>
            <input
              id="icon-file"
              type="file"
...
const defaultIconUrl = '/leaflet/diver-trim.svg'
...
const handleUpload = async () => {
  ...
  try {
    ...
    const response = await postWithToken<User, FormData>('/v1/users/icon', formData, {}, null)

    success.value = 'Icon uploaded successfully!'
...
const handleReset = async () => {
  ...
  try {
    const response = await deleteWithToken<User>('/v1/users/icon')
    success.value = 'Icon reset to default.'
    resetLocalState()
    emit('updated', response.data)
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Failed to reset icon'
    error.value = message
    console.error('Icon reset error:', err)
  } finally {
    isLoading.value = false
  }
}
